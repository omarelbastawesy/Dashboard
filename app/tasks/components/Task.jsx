...
import { faPlus, faCircleCheck, faClock, faFlag, faFire } from "@fortawesome/free-solid-svg-icons";
            ...
            <div className="flex flex-col gap-2">
              {tasks
                ...
                .map((task) => (
                  <div
                    key={task.id}
                    className="group p-6 rounded-3xl bg-(--bg-card)/40 backdrop-blur-3xl border border-white/10 hover:border-(--color-primary)/30 transition-all duration-500 shadow-xl cursor-grab active:cursor-grabbing hover:-translate-y-1 relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                      ...
                      >
                        <FontAwesomeIcon
                          icon={
                            task.status === "Done" ? faCircleCheck : faClock
                          }
                          className="text-[10px]"
                        />
                        {task.time}
