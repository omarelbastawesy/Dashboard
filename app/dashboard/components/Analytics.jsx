...
import { faChartPie, faArrowUp, faArrowDown, faBolt, faShieldHalved } from "@fortawesome/free-solid-svg-icons";

export default function Analytics() {
  return (
    <div className="lg:col-span-2 p-10 rounded-[3rem] bg-(--bg-card)/40 backdrop-blur-3xl border border-(--border-color) shadow-2xl relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex gap-2">
            <span className="px-3 py-1 bg-(--color-primary)/10 border border-(--color-primary)/20 rounded-lg text-[9px] font-black text-(--color-primary) uppercase tracking-widest">
              Live Feed
            </span>
          </div>
        </div>
        ...
        <div className="h-64 w-full flex items-end gap-2 mb-8 px-4">
          {[40, 65, 45, 80, 55, 90, 70, 85, 60, 95, 75, 88].map((h, i) => (
            ...
            >
              ...
              <span className="text-[8px] font-black text-(--text-muted) group-hover:text-white transition-colors uppercase">
                {8 + i}:00
              </span>
            </div>
          ))}
