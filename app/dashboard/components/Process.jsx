...
export default function Process() {
  return (
    <div className="space-y-8 flex flex-col gap-4">
      ...
      <div className="sm:p-8 p-4 sm:rounded-[2.5rem] rounded-lg bg-(--bg-card) border border-(--border-color) shadow-xl overflow-hidden relative group">
        ...
        <div className="flex items-center justify-between relative z-10 pb-2">
          <div>
            <p className="text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-(--text-muted) mb-1">
              Hardware Health
            </p>
        ...
        <div className="mt-6 flex items-center gap-4 relative z-10">
          ...
          <span className="text-[10px] sm:text-xs font-black text-(--info)">
            92%
          </span>
        </div>
      </div>
      ...
      {/* Quick Actions List */}
      <div className="sm:p-8 p-4 sm:rounded-[2rem] rounded-lg bg-(--bg-card)/40 backdrop-blur-3xl border border-(--border-color) flex flex-col gap-4">
        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-[0.4em] text-(--text-muted) mb-2 px-2">
          Operational Tasks
        </h4>
        <button className="flex items-center justify-between p-4 rounded-2xl bg-(--bg-card) border border-(--border-color) hover:border-(--color-primary)/30 hover:bg-(--color-primary)/5 transition-all text-left group">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[11px] sm:text-xs font-black text-(--text-primary) tracking-widest uppercase italic">
              Node Reconstruction
            </span>
          </div>
          <FontAwesomeIcon
            icon={faClock}
            className="text-[10px] sm:text-xs text-(--text-muted) group-hover:text-white transition-colors"
          />
        </button>
        <button className="flex items-center justify-between p-4 rounded-2xl bg-(--bg-card) border border-(--border-color) hover:border-(--color-primary)/30 hover:bg-(--color-primary)/5 transition-all text-left group">
          <div className="flex items-center gap-4">
