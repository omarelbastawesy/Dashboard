...
export default function Overview() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 sm:gap-6 gap-4">
      {[
        {
          name: "Project Matrix",
          icon: faCubes,
        ...
        <Link
          key={mod.name}
          href={mod.link}
          className="sm:p-8 p-4 sm:rounded-3xl rounded-lg bg-(--bg-card)/40 backdrop-blur-3xl border border-(--border-color) hover:border-(--color-primary)/40 transition-all duration-500 group relative overflow-hidden shadow-xl"
        >
          ...
          <h3 className="text-lg sm:text-xl font-black text-(--text-primary) tracking-tight mb-2 group-hover:text-(--color-primary) transition-colors">
            {mod.name}
          </h3>
          <p className="text-[10px] sm:text-xs font-bold text-(--text-muted) uppercase tracking-widest leading-relaxed">
            {mod.desc}
          </p>
        </Link>
      ))}
